...
class Notifier
{
    ...
    {
        $filesystem = $this->getS3Filesystem();
        if (!$filesystem) {
            Log::warn("Failed to backup database.  Unable to load S3 filesystem.");
            return false;
        }

        $backup_data = [];
        foreach ($this->store->findAllStates() as $state) {
            $entry = json_decode(json_encode($state), true);
            if (!$entry) {
                continue;
            }
            unset($entry['id']);
        ...
        $json_string = json_encode($backup_data);
        $filesystem->put('backup.json', $json_string);
    }
    ...
    public function restoreDatabase()
    {
        $filesystem = $this->getS3Filesystem();
        if (!$filesystem) {
            Log::warn("Failed to restore database.  Unable to load S3 filesystem");
            return false;
        }
        $json_string = $filesystem->read('backup.json');
        if (!$json_string) {
            return false;
        }
        $backup_data = json_decode($json_string, true);

        $this->store->destroyDatabase();
        ...
        foreach($backup_data as $entry) {
            ...
            $this->store->newState($check_id, $entry);
        }

        Log::debug("database restored");
        return true;
    }
